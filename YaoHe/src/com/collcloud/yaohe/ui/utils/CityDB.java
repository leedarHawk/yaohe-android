...
public class CityDB {
	...
	private static final String CITY_TABLE_NAME_NEW = "ccity";
	...
	public List<City> getNewAllCity() {
		List<City> list = new ArrayList<City>();
		Cursor c = db.rawQuery("SELECT * from " + CITY_TABLE_NAME_NEW, null);
		while (c.moveToNext()) {
			String cityID = c.getString(c.getColumnIndex("_id"));
			String city = c.getString(c.getColumnIndex("city"));
			String firstPY = c.getString(c.getColumnIndex("firstpy"));
			City item = new City(cityID, city, firstPY);
			list.add(item);
		}
		return list;
	}
