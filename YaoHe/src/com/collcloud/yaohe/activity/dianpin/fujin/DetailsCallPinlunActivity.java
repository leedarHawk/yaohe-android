 ...
 * 吆喝点评列表内容
 * 
 * @ClassName DetailsCallPinlunActivity
 * @Description
 * @author CollCloud_小米
 */
public class DetailsCallPinlunActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mStrCallID = getStringExtra(IntentKeyNames.KEY_CALL_DETAILS_COMMENT_ID);
		getCallCommentInfo();
	 ...
	 */
	private void getCallCommentInfo() {
		HttpUtils http = new HttpUtils();
		RequestParams params = new RequestParams();
		params.addBodyParameter("call_id", mStrCallID);
		String uri = ContantsValues.CALL_DETAILS_COMMENT_URL + "&call_id=" + mStrCallID ;

		http.send(HttpRequest.HttpMethod.POST, uri, params,
				new RequestCallBack<String>() {
					...
					public void onFailure(HttpException error, String msg) {
						UIHelper.ToastMessage(DetailsCallPinlunActivity.this,
	...
	private void setCommentListInfo() {
		...
		mAdapter = new HomeCallDetailsAdapter(DetailsCallPinlunActivity.this,
				mList,memberId);
			...
			case 2:
				...
				if (mList != null && mList.size() > 0) {
					...
					mAdapter = new HomeCallDetailsAdapter(
							DetailsCallPinlunActivity.this, mList,memberId);
					mLvPullToRefreshView.setAdapter(mAdapter);
				}
				break;
			default:
	...
	public void onClick(View v) {
		...
		case R.id.tv_activity_call_pinglun_send:
			...
			if (Utils.isStringEmpty(Utils.strFromView(mEtPinLun))) {
				UIHelper.ToastMessage(DetailsCallPinlunActivity.this,
						"点评内容不可为空。");
			} else if (Utils.strFromView(mEtPinLun).length() < 5) {
				UIHelper.ToastMessage(DetailsCallPinlunActivity.this,
						"请至少输入5文字。");
			} else {
...
//		CCLog.e("时间格式化：",time);
	...
	private void cancelDefinedAction() {
		...
		setResult(11, intent);
		DetailsCallPinlunActivity.this.finish();
	}
